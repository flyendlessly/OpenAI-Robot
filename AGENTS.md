...
| 功能领域 | 核心文件路径 | 依赖技术 / 说明 |
| :--- | :--- | :--- |
| **CLI 入口与运行模式** | `my_openai_robot/__main__.py` | 参数解析 (text/voice/listen/history/billing)、主流程调度 |
...
| **LLM 交互与 Client** | `my_openai_robot/llm_client.py` | 官方 OpenAI SDK (Azure 适配)、消息上下文构建、流式/非流式响应 |
...
3. **音频/阻塞调用规范**：
   ...
   - 涉及网络调用的模块（LLM / STT / TTS）注意保留并完善异常捕获与重试机制。
4. **代码风格与类型约束**：
