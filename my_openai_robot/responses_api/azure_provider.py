...
class AzureResponsesProvider(BaseResponsesProvider):
    ...
    def _invoke_create(self, **request_kwargs: Any) -> Any:
        """调用底层的 responses.create 接口，在启用时自动应用重试"""
    ...
    def create_response_stream(
        ...
        **kwargs: Any,
        ...
        logger.debug("Calling Azure responses.create stream with model=%s", self.deployment)

        stream = self._invoke_create(**request_kwargs)
        for event in stream:
            # 兼容 ResponseTextDeltaEvent (event.type == "response.output_text.delta" 或 "response.text.delta")
