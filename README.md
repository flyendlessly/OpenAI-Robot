...
3. **开发进度**：
   - [x] 完成 `llm_client` 与 `config`，使用 OpenAI SDK 实现文本 CLI 调试
   - [x] 集成费用统计与预算告警
   - [ ] 接入音频 I/O，占位 WAV → 文本流程
   - [ ] 集成 Azure Speech，形成实时语音循环
...
**重要提示**：
- `AZURE_OPENAI_ENDPOINT` 必须以 `/openai/v1/` 结尾
- `AZURE_OPENAI_DEPLOYMENT` 填写你在 Azure Portal 中创建的部署名称（不是模型名）
- 成本单价需根据你实际使用的模型调整（示例为 gpt-4o-mini 价格）
...
# 自定义参数
python -m my_openai_robot "讲个笑话" --temperature 0.7 --max-tokens 256
```
...
```bash
python -m my_openai_robot --voice-turn --record-seconds 4 --save-reply-audio reply.wav
```
...
1. **检查 endpoint 格式**：必须以 `/openai/v1/` 结尾
2. **验证部署名称**：在 Azure Portal 中确认实际的 deployment 名称
3. **测试连接**：使用 `curl` 或 `Invoke-RestMethod` 验证 Azure 服务可达性

### Azure 服务终结点

| 服务 | 终结点格式示例 |
| --- | --- |
| Azure OpenAI | `https://your-resource.services.ai.azure.com/openai/v1/` |
...
检查：
- `AZURE_OPENAI_ENDPOINT` 是否正确（必须以 `/openai/v1/` 结尾）
- `AZURE_OPENAI_DEPLOYMENT` 是否与 Azure Portal 中的部署名称一致
